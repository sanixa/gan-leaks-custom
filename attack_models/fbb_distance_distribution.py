...
def parse_arguments():
    ...
    parser.add_argument('--resolution', '-resolution', type=int, default=28,
...
def check_args(args):
    ...
    '''
    ## load dir
    print(args.gan_model_dir.strip("\r"))
    assert os.path.exists(args.gan_model_dir.strip("\r"))
    ...
    return args, save_dir, args.gan_model_dir.strip("\r")
...
def main():
    ...
    ### load query images
    pos_data_dir = args.pos_data_dir.strip("\r")
    pos_data_paths = get_filepaths_from_dir(pos_data_dir, ext='png')[: args.data_num]
    pos_query_imgs = np.array([read_image(f, resolution) for f in pos_data_paths])

    neg_data_dir = args.neg_data_dir.strip("\r")
    neg_data_paths = get_filepaths_from_dir(neg_data_dir, ext='png')[: args.data_num]
    ...
    pos_query_imgs = pos_query_imgs.reshape(args.data_num, -1)

    matrix = euclidean_distances(gen_feature, pos_query_imgs)
    idx_1 = int(args.data_num /4)
    idx_2 = int(args.data_num /2)
    idx_3 = int(args.data_num /4) + int(args.data_num /2)
    idx_1_list, idx_2_list, idx_3_list = [], [], []
    for i in range(len(gen_feature)):
        temp = np.sort(matrix[i])
        #import ipdb;ipdb.set_trace()
        idx_1_list.append(np.mean(temp[idx_1-5:idx_1+5]))
        idx_2_list.append(np.mean(temp[idx_2-5:idx_2+5]))
        idx_3_list.append(np.mean(temp[idx_3-5:idx_3+5]))


    print("POSITIVE: The nearset distance value is: %.5f " % (np.mean(pos_loss)))
    print("POSITIVE: The 25pert distance value is: %.5f " % (np.mean(idx_1_list)))
    print("POSITIVE: The 50pert distance value is: %.5f " % (np.mean(idx_2_list)))
    print("POSITIVE: The 75pert distance value is: %.5f " % (np.mean(idx_3_list)))

    ### negative query
    neg_loss, neg_idx = find_knn(nn_obj, neg_query_imgs)
    neg_query_imgs = neg_query_imgs.reshape(args.data_num, -1)

    matrix = euclidean_distances(gen_feature, neg_query_imgs)
    idx_1 = int(args.data_num /4)
    idx_2 = int(args.data_num /2)
    idx_3 = int(args.data_num /4) + int(args.data_num /2)
    idx_1_list, idx_2_list, idx_3_list = [], [], []
    ...
    print("NEGATIVE: The nearset distance value of is: %.5f " % (np.mean(neg_loss)))
    print("NEGATIVE: The 25pert distance value of is: %.5f " % (np.mean(idx_1_list)))
    print("NEGATIVE: The 50pert distance value of is: %.5f " % (np.mean(idx_2_list)))
    print("NEGATIVE: The 75pert distance value of is: %.5f " % (np.mean(idx_3_list)))
